...
float len_vec(vec2f v)
{ // длина вектора
    return sqrt(v.x * v.x + v.y * v.y);
}

vec2f normalize(vec2f v)
{ // нормализация вектора, что бы у вектора была длина 1
    float l = len_vec(v);
    v.x /= l;
    v.y /= l;
    return v;
...
void drawCircle(float x_c, float y_c, float R, unsigned char r, unsigned char g, unsigned char b)
{
    vec2f p;
    p.x = x_c;
    p.y = y_c; // p - точка центра, R - радиус, r g b - цвет
    for (int x = x_c - R - 2; x < x_c + R + 2; x++)
    {
        for (int y = y_c - R - 2; y < y_c + R + 2; y++)
        {                                                    // рассматриваем все пиксели в буффере в диапазоне от (p.x-R-2, p.y-R-2) до (p.x+R+2, p.x+R+2)
            ...
            vec2f t;
            t.x = x;
            t.y = y; // текущая обрабатываемая точка
...
void resolve(body *f, body *s, float timespan)
{
    ...
    float force = 0;
    if (distance >= EPS)
    { // во избежание деления на числа близкие нулю
...
int main(int argc, char **argv)
{
    ...
    { // 60 шагов за секунду
        ...
        resolve(&f, &s, 1. / 60.); // соответственно в качестве шага берем 1/60 секунды
    }
    ...
    {
        width = 1000;
        height = ((float)(ymax - ymin)) / (float)(xmax - xmin) * 980 + 2 * 10;
    }
    else
    {
        width = ((float)(xmax - xmin)) / (float)(ymax - ymin) * 980 + 2 * 10;
    ...
    if (fp == NULL)
    { // если файл открыть не удалось выводим сообщение об ошибке и прерываем выполнение с кодом 1
